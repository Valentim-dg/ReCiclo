...
import Achievement from "../components/Achievement"; // Componente atualizado
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
const Dashboard = forwardRef(({ user }, ref) => {
  ...
  const maxValue = Math.max(...recyclingHistory, 10); // Mínimo de 10
  ...
  // Mostrar apenas 3 conquistas por padrão: todas as desbloqueadas (até 2) mais 1 próxima a desbloquear
  const displayedAchievements = showAllAchievements
    ...
    : [
        ...
        ...lockedAchievements.slice(
          0,
          Math.max(1, 3 - unlockedAchievements.slice(0, 2).length)
        ),
      ];
            ...
            {achievements.length > 3 && (
              ...
              >
                {showAllAchievements ? (
                  ...
                  <>
                    Mostrar todas <FaChevronDown className="ml-1" />
                  </>
                )}
              </button>
            )}
